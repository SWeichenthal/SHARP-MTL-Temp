...
Take a minute to look at the dataframe to get a sense of the data structure. You can type 'View(mtl_temp_dat) in the console to do this.
...
#We maintain the database without standardized variables for mapping later
...
**Note**: *mlr* offers other methods of variable selection that are more exhaustive but take longer to run. You can try these on your own when you have more time.
...
Think about how many variables you would like to consider in your model (the example below considers all 25 but you can change this).

We will now create what *mlr* calls a "**filter wrapper**" that will determine the best set of predictors of the set of 25 we have identified. This is implemented in the following steps:
...
2.  Create the parameter space that will be searched to select variables. Here we specify we are going to search for between 4 and 25 variables. We also define a grid search algorithm which will try every value of our search space (i.e., between 4 and 25 variables). You can change these values to examine what happens to model performance if you have time (e.g., only include 5, 10, or 15 variables).
...
From the output above you can see that the filter wrapper procedure selected a model including all 25 predictors (you will see a different number if you decided to included a smaller number of variables).
...
```{r Evaluate using Cross-Validation Procedure, echo = FALSE}
...
As we did for the linear model, we will define our filter wrapper for selecting variables to include in our GAM model. Please feel free to change the number of variables you want to consider (maybe examining fewer than 25 is just fine?).
...
set.seed(2012) #Change this number
...
Dense neural networks are made up of a sequence of data processing layers. In the example below, the model includes two densely connected layers with 32 and 64 units each followed by a dropout layer (this helps with over-fitting by "dropping out" (i.e., setting to zero) a portion of the unit outputs during training ). The final layer the provides our prediction.
...
-   If you have time, you can try different hyper-parameters or trying developing models for a different outcome (like night-time temperature).
